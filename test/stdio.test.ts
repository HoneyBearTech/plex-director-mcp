...
describe("stdout stays clean for MCP", () => {
  ...
  it("a real MCP handshake gets only JSON-RPC on stdout", async () => {
    ...
    try {
      ...
      for (const name of ["check_series_completeness", "find_series_gaps", "check_series_status", "diagnose_missing_episodes", "get_upcoming_episodes"]) {
        assert.ok(tools.includes(name), `missing ${name}; tools: ${tools}`);
      }
      // README.md states this number ("22 tools", "Twenty-two tools"); change them together.
      assert.equal(tools.length, 22, `tool count changed - update the counts in README.md too. tools: ${tools}`);
    } finally {
      child.kill("SIGKILL");
      fs.rmSync(dir, { recursive: true, force: true });
    }
